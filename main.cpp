...
class payload {
    ...
    public:
        ...
        vector<unsigned int> & getMsg2() {return msg2;}
...
class IoT_ctrl_payload : public payload {
    ...
    public:
        ...
        void printPreParent(){ cout<<"pre_parent = "<< pre_parent<< "\n";}
        void setPreParent(unsigned int _id) {pre_parent=_id;}
        unsigned int getPreParent() {return pre_parent;}
};
...
class AGG_ctrl_payload : public payload {
        ...
        map<unsigned int, unsigned int> preReHop;
        unsigned int nodeParent;
    ...
    public:
        ...
        map<unsigned int,unsigned int> & getPreReHop() { return preReHop;}
        void printPreReHop()
        {
            map<unsigned int,unsigned int>::iterator iter;
            for(iter = preReHop.begin(); iter != preReHop.end(); iter++)
            {
                cout<<"preOneHop["<<iter->first<<"] = "<<iter->second<<"\n";
            }
            cout << "\n";
        }
        void setPreReHop(map<unsigned int, unsigned int> map)
        {
            preReHop = map;
        }
        
        unsigned int getNodeParent() { return nodeParent;}
        void setNodeParent(unsigned int n) { nodeParent = n;}
        void printNodeParent() { cout<< "Parent( for DIS )" << nodeParent << "\n";}
...
class DIS_ctrl_payload : public payload {
    ...
    public:
        ...
        void setNodeNewParent(unsigned int n) { nodeNewParent = n;}
        unsigned int getNodeNewParent() {return nodeNewParent;}
        unsigned int getChildrenNum() { return childrenNum;}
        void setChildrenNum( unsigned int n) { childrenNum = n;}
        void printChildrenNum() { cout<< "childrenNum: " << childrenNum << "\n";}
};
...
class node {
    ...
    public:
        ...
        // public virtual funciton to make "map<unsigned int, pair<int, unsigned int>> neighbors", it's in IoT_device, IoT_sink.
        virtual void makeNeighbor(unsigned int id, int c,unsigned int parent){};
        // public func to print children & parent.
        virtual void printParent() {}
        virtual void printChildren() {}
        virtual void printReHop() {}
        virtual void printNeighborList() {}
        virtual void printNewParent () {}
        virtual void printNodeParentList() {}
        virtual void printChildrenNumber() {}
};
...
class IoT_device: public node {
        //map<unsigned int,unsigned int> one_hop_neighbors; // you can use this variable to record the node's 1-hop neighbors
        map< unsigned int, unsigned int> reHop;
    
        bool hi; // this is used for example; you can remove it when doing hw2
    
        
        unsigned int first = 0; // 0 -> "parent" hasn't been assigned before. / 1 -> else.
        unsigned int parent = 0;
        unsigned int parent_counter = 0; // the counter of parent node. the info are from packets and will be updated in reciver.
        vector<unsigned int> children;
        map<unsigned int, pair<int, unsigned int>> neighbors; // to record the parent of neighbors. info in it are from packets and will be updated in reciver.
    ...
    public:
        ...
        // void add_one_hop_neighbor (unsigned int n_id) { one_hop_neighbors[n_id] = 0; }
        ...
        // to "set" parent
        void set_device_parent(unsigned int _id) { parent = _id;}
        // to "print" parent
        virtual void printParent(){ cout<< parent << "\n";}
    
        // COUNTER OF PARENT
        // to "get" counter of parent
        unsigned int getParentCounter() {return parent_counter;}
        // to "set" counter of parent
        void set_device_parent_counter(unsigned int _id) { parent_counter = _id;}
        // to "print" counter of parent
        void print_parent_counter(){ cout<< "parent_counter: " << parent_counter<< "\n";}
    
        // FIRST
        // to "get" first
        unsigned int get_first(){return first;}
        // to "set" first
        void set_first(unsigned int num){first=num;}
        
        // CHILDREN
        // to "print" children
        virtual void printChildren()
        {
            cout<< "children of " << getNodeID() << ": ";
            vector<unsigned int>::iterator it_i;
            for(it_i=children.begin(); it_i!=children.end(); ++it_i) cout << *it_i << " ";
            cout<< "\n";
        }
        
        // NEIGHBOR
        // to "get" neighbor
        map<unsigned int, pair<int, unsigned int>> & getNeighbors () {
            return neighbors;
        }
        // to "make" neighbor
        virtual void makeNeighbor(unsigned int id, int c,unsigned int parent)
        {
            pair<int, unsigned int> p=make_pair(c,parent);
            neighbors.insert(pair<unsigned int, pair<int, unsigned int>>(id, p));
        }
        // to "print" neighbor
        void printNeighbor()
        {
            map<unsigned int, pair<int, unsigned int>>::iterator iter;
        ...
        // ONE-HOP-NEIGHBOR
        map< unsigned int, unsigned int> & getReHop(){return reHop;}
        virtual void printReHop()
        {
            map<unsigned int, unsigned int>::iterator iter;
            for(iter = reHop.begin(); iter != reHop.end(); iter++)
                            cout<<"reHop["<<iter->first<<"] = "<<iter->second<<endl;
            cout<< "\n";
        }
    
        // NEW PARENT
        void setNewParent(unsigned int n) {newParent = n;}
        unsigned int getNewParent() {return newParent;}
        virtual void printNewParent() {cout<<"newParent of "<< getNodeID() << " = " << newParent << "\n";}
    
        // CHILD NUMBER
        unsigned int getChildrenNumber() { return childNumber;}
        void setChildrenNumber( unsigned int n) { childNumber = n;}
        virtual void printChildrenNumber() { cout<< "childrenNumber: " << childNumber << "\n";}
};
...
class IoT_sink: public node {
        ...
        map< unsigned int, unsigned int> reHop;
        map<unsigned int, vector<unsigned int>> neighborList;
        vector<unsigned int> newParentList;
        
        bool hi; // this is used for example; you can remove it when doing hw2
    
        unsigned int first = 1; // 0 -> "parent" hasn't been assigned before. / 1 -> else. It's set to 1 because sink's parent is 0 and it will not be modified in the future.
        unsigned int parent = 0; // the parent of sink is 0.
        unsigned int parent_counter = 0; // the counter of parent node. the counter of the parent of sink is set to 0.
        vector<unsigned int> children;
        map<unsigned int, pair<int, unsigned int>> neighbors; // to record the parent of neighbors. info in it are from packets and will be updated in reciver.
        unsigned int newParent = 0;
        map<unsigned int, unsigned int> nodeParentList;
        unsigned int childNumber;
    ...
    public:
        ...
        unsigned int getParent() { return parent;}
        // to "set" counter of parent
        void set_sink_parent(unsigned int _id) {parent = 0;}
        // to "print" parent
        virtual void printParent() { cout<< parent << "\n";}
        ...
        virtual void printChildren()
        {
            cout<< "children of " << getNodeID() << " (sink) : ";
            vector<unsigned int>::iterator it_i;
            for(it_i=children.begin(); it_i!=children.end(); ++it_i) cout << *it_i << " ";
            cout<< "\n";
        }
        ...
        vector<unsigned int> & getParentList() {return newParentList;}
        
        void setNewParent(unsigned int n) {newParent = n;}
        unsigned int getNewParent() {return newParent;}
        virtual void printNewParent() {cout<<"newParent of "<< getNodeID() << " = " << newParent << "\n";}
    
        map<unsigned int, unsigned int> & getNodeParentList() {return nodeParentList;}
        virtual void printNodeParentList()
        {
            cout<< "print New Parent List:\n";
            map<unsigned int, unsigned int>::iterator iter;
            for(iter = nodeParentList.begin(); iter != nodeParentList.end() ; iter++)
            {
                cout<< "new parent of " << iter->first << ": " << iter->second << "\n";
            }
        }
...
void IoT_device::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet") { // the device receives a packet from the sink
        ...
        // UPDATE CHILDREN & NEIGHBOR
        map<unsigned int, pair<int, unsigned int>> &n=getNeighbors();
        map<unsigned int, pair<int, unsigned int>>::iterator iter = n.find(p3->getHeader()->getPreID());
        // preParent: the parent of preNode stored in packet. (declear in IoT_ctrl_payload)
        // if : "PreParent's ID < neighbor[ preID ]"  ||  "neighbor[ preID ] hasn't been updateed" -> update children and neighbor
        if(iter != n.end() && (n[p3->getHeader()->getPreID()].second > l3->getPreParent() || n[p3->getHeader()->getPreID()].first == 0))
        {
            if(n[p3->getHeader()->getPreID()].second == getNodeID()) // update children: remove one child.
            {
                vector<unsigned int>::iterator it_i;
                for(it_i=children.begin(); it_i!=children.end(); ++it_i)
                {
                    if(*it_i == p3->getHeader()->getPreID())
                    {
                        children.erase(it_i);
                        break;
                    }
            ...
            if(l3->getPreParent() == getNodeID() && p3->getHeader()->getPreID() != getNodeID()) // update children: add new child.
            {
                children.push_back(p3->getHeader()->getPreID());
            }
        
            // update neighbor.
            n[p3->getHeader()->getPreID()].second = l3->getPreParent();
            n[p3->getHeader()->getPreID()].first = 1;
            
        }
        
        // CHANGE PARENT & SEND PACKET
        // if: "parent hasn't been set before"  ||  "preID's counter < parent's counter"  ||  "same counter but preID < parentID" -> change parent and sand packet.
        if(get_first()==0 ||  l3->getCounter()-1<getParentCounter()  ||  ((l3->getCounter()-1==getParentCounter()) && (getParent()> (p3->getHeader()->getPreID()) )))
        {
            set_device_parent(p3->getHeader()->getPreID());
            set_device_parent_counter(l3->getCounter()-1);
            
            if(get_first()==0)
                set_first(1);
            
            l3->setPreParent(getParent()); // set "preParent" : parent of this Node
            p3->getHeader()->setPreID ( getNodeID() ); // set "PreID" : NodeID.
            p3->getHeader()->setNexID ( BROADCAST_ID ); // set "NextID" & "DstID" : BROADCAST_ID
        ...
        // recieve one packet -> pop a children
        if(p3->getHeader()->getPreID() != getNodeID())
           children.pop_back();
        
        p3->getHeader()->setPreID ( getNodeID() );
        p3->getHeader()->setNexID ( getParent() );
        
        // if children is empty -> has recieved packets from all children. -> send packet to device's parent.
        if(children.empty())
        {
            send_handler(p3);
        }
    ...
    else if (p->type() == "AGG_ctrl_packet")
    {
        ...
        // SET "MSQ( NEIGHBOR )" & "MAP <unsigned int
        if( h3->getSrcID() == getNodeID())
        {
            const map<unsigned int, bool> &n = getPhyNeighbors();
            vector<unsigned int> &m = l3->getMsg2();
            for( map<unsigned int, bool>::const_iterator iter = n.begin() ; iter != n.end() ; iter++)
            {
                m.push_back(iter->first);
            }
            l3->setNodeParent(getParent());
        }
        // cout << "node id = " << getNodeID() << ", msg = "  << l3->getMsg() << endl;
        
        
        // SET unsigned int> ( RESERVE PATH )"
        map<unsigned int, unsigned int> & preh = l3->getPreReHop();
        map<unsigned int, unsigned int> & h = getReHop();
        map<unsigned int, unsigned int>::iterator iter;
        if(p3->getHeader()->getPreID() != getNodeID())
        {
            for(iter = preh.begin() ; iter != preh.end() ; iter++)
            {
                h.insert(pair<unsigned int,unsigned int>(iter->first, p3->getHeader()->getPreID()));
            }
            h.insert(pair<unsigned int,unsigned int>(p3->getHeader()->getPreID(),p3->getHeader()->getPreID()));
        }
        
        l3->setPreReHop(h);
        p3->getHeader()->setPreID ( getNodeID() ); // set "PreID" : NodeID.
        p3->getHeader()->setNexID ( getParent() ); // set "NextID" & "DstID" : BROADCAST_ID
        ...
        if(p3->getHeader()->getDstID() == getNodeID())
        {
            setNewParent(l3->getParent());
            setChildrenNumber(l3->getChildrenNum());
        }
        else
        {
            map<unsigned int, unsigned int> reverse_path = getReHop();
            p3->getHeader()->setPreID( getNodeID() );
            p3->getHeader()->setNexID(reverse_path[p3->getHeader()->getDstID()]);
            //l3->setNodeNewParent(l3->getNodeNewParent());
...
void IoT_sink::recv_handler(packet *p)
{
    ...
    if (p->type() == "IoT_ctrl_packet" ) { // the device receives a packet from the sink
        ...
        l3->increase();
        l3->setPreParent(0); // set "PreParent" : 0
        
        // only send packet once.
        if(!hi)
            send_handler(p3);
        hi = true;
        // unsigned mat = l3->getMatID();
        ...
        // set neighbor list
        map<unsigned int, vector<unsigned int>> & nlist = getNeighborList();
        nlist[p3->getHeader()->getSrcID()] = l3->getMsg2();
        
        map<unsigned int, unsigned int> & npl = getNodeParentList();
        npl[p3->getHeader()->getSrcID()] = l3->getNodeParent();
        
        // SET unsigned int> ( RESERVE PATH )"
        //map<unsigned int,unsigned int> one_hop_neighbors;
        map<unsigned int, unsigned int> & preh = l3->getPreReHop();
        map<unsigned int, unsigned int> & h = getReHop();
        map<unsigned int, unsigned int>::iterator iter;
       for(iter = preh.begin() ; iter != preh.end() ; iter++)
       {
        ...
        DIS_ctrl_payload *l3 = nullptr;
        l3 = dynamic_cast<DIS_ctrl_payload*> (p3->getPayload());
        vector<unsigned int> childrenN;
        
        // set neighbor list of sink
        const map<unsigned int, pair<int, unsigned int>> &n = getNeighbors();
        vector<unsigned int> sinkNeighborList; // sinkNeighborList: the neighbor list of sink
        for( map<unsigned int, pair<int, unsigned int>>::const_iterator iter = n.begin() ; iter != n.end() ; iter++)
        {
            sinkNeighborList.push_back(iter->first);
        }
        
        // set sink children num.
        setChildrenNumber(0);//(getChildrenNumber());
        
        // cout << "node id = " << getNodeID() << ", parent = "  << l3->getParent() << endl;
        map<unsigned int, vector<unsigned int>> & nlist = getNeighborList();
        nlist[0] = sinkNeighborList;
        vector<unsigned int> & newP = getParentList();
        map<unsigned int, unsigned int> & reverse_path = getReHop();
        unsigned int i=1;
        //unsigned int newp[]={0,0,0,0,1,1,2,2,3};
        map<unsigned int, unsigned int> & npl = getNodeParentList();
        for(auto iter = nlist.begin(); iter != nlist.end(); iter++)
        {
            ...
            if(++iter2 == nlist.end())
                break;
            //newP.push_back(node::id_to_node(i)->getParent());
            l3->setParent( npl[i] );
            l3->setChildrenNum( npl[i] );//( childrenN[i] );
            p3->getHeader()->setPreID( getNodeID() );
            p3->getHeader()->setNexID( reverse_path[i] );
            p3->getHeader()->setDstID( i );
            i++;
            send_handler(p3);
        ...
        /*for(int i=1;i<8;i++)
        {
            //cout<< "New: " << newp[i] << ", preID: " << getNodeID() << ", NxtID: " << reverse_path[i] << ", DstID: " << i << "\n";
            l3->setNodeNewParent(newp[i]);
            p3->getHeader()->setPreID( getNodeID() );
            p3->getHeader()->setNexID(reverse_path[i]);
            p3->getHeader()->setDstID( i );
...
int main()
{
    ...
    // link::link_generator::print(); // print all registered links
     ...
     unsigned int SimTime, BFS_Start_Time, AGG_Start_Time;
     cin >> SimTime >> BFS_Start_Time >> AGG_Start_Time;
    
     unsigned int DIS_Start_Time, Data_Trans_Time;
     cin >> DIS_Start_Time >> Data_Trans_Time;
    ...
    for(unsigned int i=0;i<Nodes;i++)
    {
     ...
     for(unsigned int i=0;i<Links;i++)
     {
         ...
         if( Link_End1 == 0 )
         {
             node::id_to_node(Link_End1)->makeNeighbor(Link_End2, 0, 0);
             node::id_to_node(Link_End2)->makeNeighbor(Link_End1, 1, 0);
         }
         else if( Link_End2 == 0 )
         {
             node::id_to_node(Link_End1)->makeNeighbor(Link_End2, 1, 0);
             node::id_to_node(Link_End2)->makeNeighbor(Link_End1, 0, 0);
         }
         else
         {
             node::id_to_node(Link_End1)->makeNeighbor(Link_End2, 0, 0);
     ...
     {
         AGG_ctrl_packet_event(i, 0, AGG_Start_Time);
     }
     ...
     {
         cout << i << " ";
         node::id_to_node(i)->printParent();
         //node::id_to_node(i)->printChildrenNumber();
     }
     
     //node::id_to_node(0)->printNodeParentList();
     //node::id_to_node(0)->printNeighborList();
    
    /*int Nodes, Links;
    cin >> Nodes >> Links;
    int SimTime, BFS_Start_Time, Data_Trans_Time;
    ...
    {
        IoT_data_packet_event(i, 0, 200);
        //AGG_ctrl_packet_event(i, 0, 250);
    }
    ...
    //IoT_data_packet_event(4, 0, Data_Trans_Time);
    ...
    {
        //IoT_data_packet_event(i, 0, Data_Trans_Time);
        AGG_ctrl_packet_event(i, 0, 50);//250);
    }
    ...
    event::start_simulate(300);//(SimTime);
    // event::flush_events() ;
    // cout << packet::getLivePacketNum() << endl;
    
    // print every node's parent
    for(int i=0;i<Nodes;i++)
    {
        cout << i << " ";
        node::id_to_node(i)->printParent();
        node::id_to_node(i)->printReHop();
        node::id_to_node(i)->printNewParent();
    }
